...
let type_union : t -> t -> t = failwith ""
let type_of_const : const -> t = failwith ""
let type_less : t -> t -> bool = failwith ""
...
let rec specialise_bb : basic_blocks -> label -> (pe_state, label) MState.state =
    ...
    let prepare_phi : (var, var) Smap.smap -> unit =
        let iter_do (reg, from) =
            ...
            slots'.(i_reg) <- {typ = type_union v1.typ v2.typ; value=v2.value}
...
and specialise :  func_def (* current function *)
    ...
    let _ = m.run_state init_state in 
    failwith ""
